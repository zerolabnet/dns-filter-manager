...
* **Интеграция с AdGuard Home:**
    * Приложение управляет единым списком `filter.list` и после сохранения дергает `POST /control/filtering/refresh` для обновления фильтрации в AdGuard Home.
    * Создавайте и редактируйте списки блокировки доменов: изменения мгновенно применяются в AdGuard Home после refresh.
